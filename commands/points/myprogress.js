...
module.exports.run = async (client, message, args, level, Discord) => {
    ...
    if (message.member.roles.cache.some((r) => r.name.includes('Team') && excessiveCharacterArray.includes(r.name.substr(r.name.indexOf(' ')+1)))) {
        let role = message.member.roles.cache.find((r) => r.name.includes('Team') && excessiveCharacterArray.includes(r.name.substr(r.name.indexOf(' ')+1)));

	if (message.member.roles.cache.some((r) => r.name.includes('Garfield')))
	    role = message.member.roles.cache.find((r) => r.name.includes('Garfield'));
	if (message.member.roles.cache.some((r) => r.name.includes('aerith')))
	    role = message.member.roles.cache.find((r) => r.name.includes('aerith'));
