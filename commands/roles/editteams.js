...
module.exports.run = async (client, message, args, level) => {
  ...
  if (args[0] > 13 && args[0] != 999) { return message.error('Invalid level', 'The second argument of the command must be a number -1 to 11.'); }
  ...
  if(args[0] == 0)
  		...
  		client.teamSettings.push('secretTeams', name);
  	else {
...
module.exports.help = {
  ...
  details: '<rank> => 0 = no rank required, 1-11 = that rank, 12 = rank users cannot get themselves (i.e. Garfield & aerith), 13 = secret teams, 999 = Delete the team from selection',
};